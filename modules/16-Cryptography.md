...
> **Hash** - One-way encryption that produces a fix-length string (hash) based on the arrangement of data bits in the input. Provides integrity and confidentiality.

Hashes are impossible to recover the original message from the digest and is used to store passwords providing confidentiality.
